...
int Optimizer::PoseOptimization(Frame* pFrame, optInfo optParams)
{
    ...
    int nStereoVert = 0;  // number of stereo visual vertices in graph - for normalization
    for(int i=0; i<N; i++)
    {
       MapPoint* pMP = pFrame->hasAssociation(i);
       if(pMP){
          if(views.uR(i)<0){ nMonoVert++;} //monocular
          else {nStereoVert++;} //stereo
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(views.uR(i)<0)
            {
                ...
                pFrame->setOutlier(i, false);//this should be redundant since pts start off as inliers

                Eigen::Matrix<double,2,1> obs;
                const cv::KeyPoint &kpUn = views.keypt(i);
      ...
      //          e->setInformation(Eigen::Matrix2d::Identity()*invSigma2/nMonoVert);
                ...
                const Camera camera = pFrame->getCamera();
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw));
        optimizer.initializeOptimization(0);  // only include edge of level 0 (i.e. level 1, the outliers, are excluded).
...
*/
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const cv::KeyPoint &kpUn1 = pKF1->getViews().keypt(i);
        ...
        const float &invSigmaSquare1 = pKF1->getViews().orbParams().mvInvLevelSigma2[kpUn1.octave];
        ...
        const cv::KeyPoint &kpUn2 = pKF2->getViews().keypt(i2);
        ...
        float invSigmaSquare2 = pKF2->getViews().orbParams().mvInvLevelSigma2[kpUn2.octave];

...
bool LoopClosing::DetectLoop()
{
  ...
  //  const vector<KeyFrame*> vpConnectedKeyFrames = mpCurrentKF->GetVectorCovisibleKeyFrames();
    const vector<KeyFrame*> vpConnectedKeyFrames = maps[curKF_cam]->getKeyFrameDB()->GetVectorCovisibleKeyFrames(mpCurrentKF);
  ...
  //  vector<KeyFrame*> vpCandidateKFs = mpKeyFrameDB->DetectLoopCandidates(mpCurrentKF, minScore);
    vector<KeyFrame*> vpCandidateKFs = maps[curKF_cam]->getKeyFrameDB()->DetectLoopCandidates(mpCurrentKF, minScore);
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
     ...
     //   set<KeyFrame*> spCandidateGroup = pCandidateKF->GetConnectedKeyFrames();
        set<KeyFrame*> spCandidateGroup =   maps[curKF_cam]->getKeyFrameDB()->GetConnectedKeyFrames(pCandidateKF);
...
bool LoopClosing::ComputeSim3()
{
   ...
   // vector<KeyFrame*> vpLoopConnectedKFs = mpMatchedKF->GetVectorCovisibleKeyFrames();
    vector<KeyFrame*> vpLoopConnectedKFs = maps[curKF_cam]->getKeyFrameDB()->GetVectorCovisibleKeyFrames(mpMatchedKF);
...
void LoopClosing::CorrectLoop()
{
   ...
   // mvpCurrentConnectedKFs = mpCurrentKF->GetVectorCovisibleKeyFrames();
    ...
    {
        // Get Map Mutex
        unique_lock<mutex> lock(maps[curKF_cam]->mMutexMapUpdate);
    ...
    for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
    {
       ...
       // vector<KeyFrame*> vpPreviousNeighbors = pKFi->GetVectorCovisibleKeyFrames();
        vector<KeyFrame*> vpPreviousNeighbors = maps[curKF_cam]->getKeyFrameDB()->GetVectorCovisibleKeyFrames(pKFi);
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF)
{
    ...
    //fix KeyFrame = 0;
    vector<KeyFrame*> vpAllKFs = maps[curKF_cam]->GetAllKeyFrames();
    for(vector<KeyFrame*>::const_iterator vit = vpAllKFs.begin(); vit != vpAllKFs.end(); ++vit){
         KeyFrame* pKFi = *vit;
         if(pKFi->mnId == 0){
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            ...
            list<KeyFrame*> lpKFtoCheck(maps[curKF_cam]->mvpKeyFrameOrigins.begin(),maps[curKF_cam]->mvpKeyFrameOrigins.end());

            while(!lpKFtoCheck.empty())
            {
                KeyFrame* pKF = lpKFtoCheck.front();
               // const set<KeyFrame*> sChilds = pKF->GetChilds();
                const set<KeyFrame*> sChilds = maps[curKF_cam]->getKeyFrameDB()->getChildren(pKF);
            ...
            // Correct MapPoints
            const vector<MapPoint*> vpMPs = maps[curKF_cam]->GetAllMapPoints();
